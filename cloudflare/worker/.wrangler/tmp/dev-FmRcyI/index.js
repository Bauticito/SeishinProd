...
var getIdFromPath = /* @__PURE__ */ __name((url) => {
...
var isAdmin = /* @__PURE__ */ __name((request, env) => {
...
var toStringOrNull = /* @__PURE__ */ __name((value) => {
...
var src_default = {
  async fetch(request, env) {
    ...
    if (request.method === "POST" && url.pathname === "/api/media") {
      ...
      const insert = await env.DB.prepare(
        `INSERT INTO media_items (title, media_type, src, description, sort_order, is_published)
         VALUES (?, ?, ?, ?, ?, ?)`
      ).bind(title, mediaType, src, description, sortOrder, isPublished).run();
    ...
    if (request.method === "PUT" && /^\/api\/media\/\d+$/.test(url.pathname)) {
      ...
      const isPublished = typeof body.is_published === "number" && (body.is_published === 0 || body.is_published === 1) ? body.is_published : null;
      ...
      const update = await env.DB.prepare(
        `UPDATE media_items
         ...
         WHERE id = ?`
      ).bind(title, mediaType ?? null, src, description, sortOrder, isPublished, id).run();
...
// .wrangler/tmp/bundle-Fm5ixe/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Fm5ixe/middleware-loader.entry.ts
