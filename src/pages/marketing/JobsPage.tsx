...
export default function JobsPage() {
  ...
  useEffect(() => {
    document.title = `${t('nav.inicio')} | Bolsas de Trabajo y Vacantes | Seishin International`;
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) {
      metaDesc.setAttribute('content', 'Explora las vacantes actuales en Seishin International. Únete a nuestro equipo en Aguascalientes y Guanajuato. Oportunidades en Ingeniería, IA e Industria 4.0.');
    }
  }, [t]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6">
          {loading ? (
            ...
            finalJobs.map((job: JobVacancy, index: number) => (
              <motion.div
                key={job.id}
            ...
            <div className="text-center py-20 bg-white/5 rounded-2xl border border-dashed border-white/10">
              <p className="text-[var(--text-secondary)]">No hay vacantes activas en este momento.</p>
            </div>
          )}
        </div>

        {/* Pagination / More info */}
        <div className="mt-16 p-8 rounded-3xl bg-gradient-to-r from-[#E31E24]/10 to-transparent border border-[#E31E24]/20 text-center">
