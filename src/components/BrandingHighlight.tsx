...
export default function BrandingHighlight() {
  ...
  const items = [Cpu, Globe, Zap].map((icon, index) => ({
    icon,
    title: t(`branding.items.${index}.title`),
    desc: t(`branding.items.${index}.desc`),
  }));

  return (
    <section className="py-24 relative overflow-hidden bg-[var(--bg-primary)]">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[#E31E24] rounded-full blur-[160px] opacity-25 animate-pulse" />
        <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-[#3A3A3A] rounded-full blur-[140px] opacity-20" />
      </div>
        ...
        <div className="bg-[var(--card-bg)] backdrop-blur-3xl rounded-[3rem] p-12 md:p-24 border border-[var(--border-color-light)] text-center shadow-2xl relative overflow-hidden">
          ...
          >
            ...
            <p className="text-2xl md:text-3xl text-[var(--text-secondary)] font-medium max-w-4xl mx-auto mb-16 leading-relaxed">
              {t('branding.line_1')} <span className="text-[#E31E24]">{t('branding.line_2')}</span> {t('branding.line_3')}
            </p>
