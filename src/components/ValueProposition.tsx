...
const valueIcons = [CheckCircle, Users, Award, Lightbulb, Shield, Bot];

export default function ValueProposition() {
  const { t } = useTranslation();
  const values = valueIcons.map((icon, index) => ({
    icon,
    title: t(`value.items.${index}.title`),
    description: t(`value.items.${index}.description`),
  }));
  const propositions = [0, 1, 2, 3, 4].map((index) => t(`value.propositions.${index}`));
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-[var(--text-primary)] mb-4 tracking-tight leading-tight">
            {t('value.heading')} <span className="gradient-text">{t('value.heading_accent')}</span>
          </h2>
          <p className="text-lg sm:text-xl text-[var(--text-secondary)] max-w-3xl mx-auto">{t('value.intro')}</p>
        </div>
          ...
          {values.map((value, index) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-lg sm:text-xl font-bold text-[var(--text-primary)] mb-2">{value.title}</h3>
                <p className="text-[var(--text-secondary)] leading-relaxed text-sm sm:text-base">{value.description}</p>
              </div>
            );
          })}
        </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
            {propositions.map((prop, index) => (
              <div key={index} className="flex items-start gap-3">
                ...
                <p className="text-[var(--text-secondary)] text-base sm:text-lg leading-relaxed">{prop}</p>
              </div>
            ))}
          </div>
        </div>
